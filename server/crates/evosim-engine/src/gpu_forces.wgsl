// WebGPU compute shader (WGSL) for the per-particle force pass.
// Ported verbatim from src/sim/gpu-forces-shader.ts -- the WGSL math
// is identical, only the host bindings differ. Brownian noise is
// counter-based PCG keyed by (particleIdx, tickSeed) so the GPU and
// rayon-CPU paths land in the same probability distribution per slot.
