...
pub fn run_particle_decay(store: &mut ParticleStore, dt: f32) -> usize {
    let mut removed = 0;
    let mut i = store.len();
    while i > 0 {
        i -= 1;
        store.age[i] += dt;
        store.r[i] = (store.r[i] - RADIUS_SHRINK_PER_S * dt).max(0.0);
...
mod tests {
    ...
    fn removes_at_max_age() {
        ...
        let removed = run_particle_decay(&mut s, MAX_AGE_S + 1.0);
        assert_eq!(removed, 1);
        assert_eq!(s.len(), 0);
    }
    ...
    fn removes_tiny_particles() {
        ...
        let removed = run_particle_decay(&mut s, 1.0);
        assert_eq!(removed, 1);
        assert_eq!(s.len(), 0);
    }
