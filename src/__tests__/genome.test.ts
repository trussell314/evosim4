...
describe("genome decoding: known byte sequences", () => {
  ...
  describe("viableGenome structural gate", () => {
    ...
    it("FOUNDER_GENE_REFS mirror the canonical chem/slot constants", async () => {
      ...
      expect(F.ACT_LIGHT_X).toBe(c.CHEM_ACT_LIGHT_X);
      expect(F.ACT_LIGHT_Y).toBe(c.CHEM_ACT_LIGHT_Y);
      expect(F.ACT_VIB_X).toBe(c.CHEM_ACT_VIB_X);
      expect(F.ACT_VIB_Y).toBe(c.CHEM_ACT_VIB_Y);
      expect(F.SYNTH_VIBRO).toBe(r.RX_SLOT_SYNTH_VIBRO);
